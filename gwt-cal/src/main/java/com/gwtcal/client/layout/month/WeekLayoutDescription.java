 ...
 * @see com.gwtcal.client.MonthCalendar
 * @see com.gwtcal.client.layout.month.MonthLayoutDescription
 */
public class WeekLayoutDescription<T> {
   ...
   private DayLayoutDescription<T>[] days = null;
   ...
   public WeekLayoutDescription(AppointmentProvider<T> appointmentProvider,
                                Date calendarFirstDay, Date calendarLastDay, int maxLayer) {
   ...
   public WeekLayoutDescription(AppointmentProvider<T> appointmentProvider, Date calendarFirstDay, Date calendarLastDay) {
      this(appointmentProvider, calendarFirstDay, calendarLastDay, Integer.MAX_VALUE);
   }
   ...
   private DayLayoutDescription<T> initDay(int day) {
      assertValidDayIndex(day);
      if (days[day] == null) {
         days[day] = new DayLayoutDescription<T>(appointmentProvider, day);
      }
      return days[day];
   }
   ...
   public void addAppointment(T appointment) {
      int dayOfWeek = dayInWeek(appointmentProvider.getStart(appointment));
      if (appointmentProvider.isAllDay(appointment)) {
         topAppointmentsManager.assignLayer(
            new AppointmentLayoutDescription<T>(dayOfWeek, appointment));
      } else {
   ...
   public void addMultiDayAppointment(T appointment) {
      int weekStartDay = dayInWeek(appointmentProvider.getStart(appointment));
      int weekEndDay = dayInWeek(appointmentProvider.getEnd(appointment));

      if(!appointmentProvider.getEnd(appointment).before(calendarLastDay)) {
    	  weekEndDay = LAST_DAY;
      }

      topAppointmentsManager.assignLayer(
         new AppointmentLayoutDescription<T>(weekStartDay, weekEndDay, appointment));
   }

   public void addMultiWeekAppointment(T appointment,
      AppointmentWidgetParts presenceInMonth) {
	   ...
	   switch (presenceInMonth) {
         case FIRST_WEEK:
            int weekStartDay = dayInWeek(appointmentProvider.getStart(appointment));
            topAppointmentsManager.assignLayer(
               new AppointmentLayoutDescription<T>(weekStartDay, LAST_DAY, appointment));
            break;
         case IN_BETWEEN:
            topAppointmentsManager.assignLayer(
               new AppointmentLayoutDescription<T>(FIRST_DAY, LAST_DAY, appointment));
            break;
         case LAST_WEEK:
            int weekEndDay = dayInWeek(appointmentProvider.getEnd(appointment));
            topAppointmentsManager.assignLayer(
               new AppointmentLayoutDescription<T>(FIRST_DAY, weekEndDay, appointment));
            break;
      }
   }
